...
public class FeedbackPuzzleManager : MonoBehaviour
{
    [Header("Painéis")]
    ...
    [Header("Navegação - Retornar (PUZZLE)")]
    [SerializeField] private string cenaNiveis = "CenaNiveis";
    [SerializeField] private ModoDeJogoData modoPuzzleData; // <- arraste aqui o ScriptableObject do Puzzle
    ...
    void Start()
    {
        ...
        int movimentosMaximosDoNivel = PlayerPrefs.GetInt("MovimentosMaximosDoNivel", 100);
        ...
        if (resultado == "vitoria")
        {
            ...
            int moedasGanhaas = 15 + (int)Mathf.Max(0, (movimentosMaximosDoNivel - movimentos) * 0.5f);
            if (moedasGanhaas > 60) moedasGanhaas = 60;

            int bonusPorMovimentos = (movimentosMaximosDoNivel - movimentos) * 5;
            int bonusPorTempo = (int)Mathf.Max(0, 180 - tempoFinal) * 2;
            ...
            if (pontuacaoFinal > pontuacaoAntiga)
            {
                ...
                PlayerDataManager.Instance.Dados.PontuacaoTotalPuzzle += diferencaDePontos;
    ...
    public void TentarNovamente()
    {
        ...
        {
            Debug.LogError("Não foi possível encontrar a última cena jogada! Voltando aos níveis do Puzzle.");
            RetornarAoMenu();
        }
    }
    ...
    public void RetornarAoMenu()
    {
        ...
        {
            Debug.LogError("modoPuzzleData não foi definido no FeedbackPuzzleManager. Voltando ao menu principal por segurança.");
    ...
    {
        GameDataHolder.NivelParaCarregar = null;
        RankingState.TabParaAbrir = RankingTab.Puzzle;
